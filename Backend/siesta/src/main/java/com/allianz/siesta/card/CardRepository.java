...
public interface CardRepository extends JpaRepository <Card, Long> {
    ...
    @Query(value = "SELECT SUM(COALESCE(estimate_time,0.0)) AS totalEstimateTime, SUM(COALESCE(actual_time,0.0)) AS totalActualTime, " +
            "DATE(card_date) AS cardDate " +
            "FROM card " +
            "WHERE deleted_status IS FALSE " +
            "AND user_id = :id " +
            "GROUP BY DATE(card_date) ", nativeQuery = true)
    List<Object[]> getTotalEstimateAndActualTimeGroupByCardDate(@Param("id") Long id);

    @Query(value = "SELECT c.id, c.card_name, c.estimate_time, c.actual_time, c.create_date, a.assignment_name FROM card c, assignment a " +
